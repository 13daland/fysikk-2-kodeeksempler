...
P1 = np.array([-0.05, 0])       # posisjon til kule 1, m
q2 = -2.00e-9                   # ladning til kule 2, C
P2 = np.array([0.05, 0])        # posisjon til kule 2, m
...
def E_felt(r):
    ...
    r1_enhet = r1/np.linalg.norm(r1)            # enhetsvektor ut fra P1
    E1 = k*q1/np.linalg.norm(r1)**2 *r1_enhet   # elektrisk felt fra kule 1
    r2 = r - P2                                 # vektor fra P2 til r
    r2_enhet = r2/np.linalg.norm(r2)            # enhetsvektor ut fra P2
...
# Beregning
punkt = np.array([0, 0.10])         # punktet der vi beregner feltet
E = E_felt(punkt)                   # beregning av felt

# Vi skriver ut resultatet
print("Det elektriske feltet i punktet", punkt, "er", E, ".")
print("Det har en absoluttverdi på", np.linalg.norm(E), "V/m.")
print("Vinkelen mellom feltretningen og x-aksen er", np.arctan(E[1]/E[0])/math.pi*180, "grader.")